...
function buildCoverSheet(payload: ExportPayload): XLSX.WorkSheet {
  ...
  push(
    padRow(
      [
        {
          v: "Report generated by Basket Statistics",
...
export function exportToPdf(payload: ExportPayload): void {
  ...
  const addHeader = () => {
    ...
    doc.setFontSize(16)
    doc.text("Signing Advisor — Report", margin, 28)
  ...
  doc.setFont("helvetica", "bold")
  doc.setFontSize(13)
  doc.text(`Team: ${team.name}`, margin, y)
  y += 18
  doc.setFont("helvetica", "normal")
  doc.setFontSize(10)
  doc.text(`League: ${team.leagueName ?? team.leagueSlug}`, margin, y)
  y += 14
  doc.text(
    `Messages: ${messages.length}    Recommendations: ${recs.length}`,
    margin,
    y,
  )
  y += 18
  ...
  if (last) {
    ...
    doc.setFont("helvetica", "bold")
    doc.setFontSize(12)
    doc.text("Team diagnosis", margin, y)
    y += 14
    doc.setFont("helvetica", "normal")
    doc.setFontSize(10)
    y =
    ...
    y =
      drawWrappedText(
        doc,
        `Current core: ${last.team.topPlayers.join(", ") || "—"}`,
        margin,
        y,
        pageWidth - margin * 2,
        10,
  ...
  doc.setFont("helvetica", "bold")
  doc.setFontSize(12)
  doc.text("Player profiles — Recommended candidates", margin, y)
  y += 6

  if (recs.length === 0) {
    doc.setFont("helvetica", "italic")
    doc.setFontSize(10)
    doc.text("No candidates in this conversation.", margin, y + 14)
    y += 24
  } else {
    autoTable(doc, {
      startY: y + 4,
    ...
    for (const [i, r] of recs.entries()) {
      ...
      doc.setFont("helvetica", "bold")
      doc.setFontSize(11)
      doc.text(`Player ${i + 1} — ${r.name}`, margin, y)
      y += 14
      doc.setFont("helvetica", "normal")
      doc.setFontSize(9)
      const lines: string[] = [
        `Position: ${r.position}    League: ${r.league}    Age: ${r.age} years    Contract: ${r.contractValue}`,
        `Market: ${r.market}    Priority: ${r.priority}`,
        `Fit: ${r.fit}`,
  ...
  if (last && last.considerations.length > 0) {
    ...
    doc.setFont("helvetica", "bold")
    doc.setFontSize(12)
    doc.text("Before negotiating", margin, y)
    y += 14
    doc.setFont("helvetica", "normal")
    doc.setFontSize(10)
    for (const c of last.considerations) {
  ...
  doc.setFont("helvetica", "bold")
  doc.setFontSize(14)
  doc.text("Full conversation", margin, y)
  y += 16
  ...
  if (messages.length === 0) {
    ...
    for (const [i, m] of messages.entries()) {
      ensureSpace(50)
      const label = m.type === "user" ? "User" : "Advisor"
      ...
      doc.setFont("helvetica", "bold")
      doc.setFontSize(9)
      doc.text(`${label} · message ${i + 1}`, margin + 6, y + 4)
      doc.setTextColor(0, 0, 0)
      y += 22
...
export async function exportToWord(payload: ExportPayload): Promise<void> {
  ...
  children.push(
    metaParagraph("Generated:", formatDate(generatedAt)),
    metaParagraph("Team:", team.name),
    metaParagraph("League:", team.leagueName ?? team.leagueSlug),
    metaParagraph("Messages:", String(messages.length)),
    metaParagraph("Recommendations:", String(recs.length)),
    spacer(),
  )

  if (last) {
    children.push(
      sectionHeading("Team diagnosis"),
      bodyParagraph(`${last.intentEmoji} ${last.intentLabel}`),
      bodyParagraph(stripMarkdown(last.analysis)),
      bodyParagraph(`Gap detected: ${last.gap}`),
      bodyParagraph(`Current core: ${last.team.topPlayers.join(", ") || "—"}`),
      spacer(),
    )
  }

  children.push(sectionHeading("Player profiles — Recommended candidates"))
  ...
  if (recs.length === 0) {
    children.push(
      new Paragraph({
        children: [
          new TextRun({
            text: "No candidates in this conversation.",
            italics: true,
          }),
        ],
      }),
    ...
    for (const [i, r] of recs.entries()) {
      children.push(
        ...
        bodyParagraph(`Market: ${r.market}    Priority: ${r.priority}`),
        bodyParagraph(`Fit: ${r.fit}`),
        bodyParagraph(`Strengths: ${r.strengths.join(" • ") || "—"}`),
      )
    }
  }
  ...
  if (last && last.considerations.length > 0) {
    children.push(
      spacer(),
      sectionHeading("Before negotiating"),
      ...last.considerations.map((c) => bodyParagraph(`• ${c}`)),
    )
  }

  children.push(spacer(), sectionHeading("Full conversation"))
  ...
  if (messages.length === 0) {
    ...
    messages.forEach((m, i) => {
      ...
      const label = isUser ? "User" : "Advisor"
  ...
  const doc = new Document({
    creator: "Basket Statistics",
    title: `Signing Advisor Report — ${team.name}`,
    description: "Conversation and recommended player profiles",
    sections: [
      {
        properties: {},
        children,
