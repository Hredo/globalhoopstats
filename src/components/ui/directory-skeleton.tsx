...
export function DirectorySkeleton({ cards = 12 }: { cards?: number }) {
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="mt-6 h-14 animate-pulse rounded-lg border border-white/5 bg-white/[0.02]" />

        <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3">
          {Array.from({ length: cards }).map((_, i) => (
            <div
              key={i}
              className="rounded-lg border border-white/5 bg-white/[0.02] p-4 sm:p-5"
            >
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="h-14 w-14 animate-pulse rounded-md bg-white/5 sm:h-16 sm:w-16" />
